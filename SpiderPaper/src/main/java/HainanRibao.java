 ...
 */
public class HainanRibao {

    SaveXML savexml = new SaveXML();

    public HainanRibao( String newspaper ) {
    ...
    public void getNewsInfo( String NewsUrl ) { 	//获得新闻来源URL
        try {
            ...
            Document Doc = Jsoup.connect(NewsUrl)
                    ...
                    .timeout(3000)
                    .get();
            Element TitleEle = Doc.select("p[class=BSHARE_TEXT]").first();
            ...
            Elements IMGs = Doc.select("a[class=pirobox_gall]").select("img[src]");
