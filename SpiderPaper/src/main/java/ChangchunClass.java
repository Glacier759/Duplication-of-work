 ...
 */
public class ChangchunClass {

    SaveXML savexml = new SaveXML();

    public ChangchunClass( String newspaper ) {
    ...
    public void getNewsInfo( String NewsUrl ) { 	//获得新闻来源URL
        try {
            ...
            Element TitleEle = textDIV.select("td[class=font01]").first();
            String Title = TitleEle.text(); 		//获得文章title

            String PublishTime = getDate(NewsUrl); 	//获得文章发表日期
            Elements ContentPTags = textDIV.select("founder-content").select("p");
            String Content = "\r\n"; 						//获得文章正文内容
