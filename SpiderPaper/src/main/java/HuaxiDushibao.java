 ...
 */
public class HuaxiDushibao {

    SaveXML savexml = new SaveXML();

    public HuaxiDushibao( String newspaper ) {
    ...
    public HashMap<String,String> getLayout( String Link ) {
        ...
        try {
            Document Doc = Jsoup.connect(Link)
                    ...
                    .get();
            //获取版面链接
            Elements Layouts = Doc.select("div[id=content]").select("li[onclick]");
            for (Element Layout:Layouts) {
                String onclick = Layout.attr("onclick");
                String href = Link+onclick.substring(onclick.indexOf('\'')+2, onclick.lastIndexOf('\''));
                LayoutMap.put(href, Layout.text());
            }
    ...
    public void getNewsInfo( String NewsUrl ) { 	//获得新闻来源URL
        try {
            ...
            Elements IMGs = Doc.select("div[class=bt_con]").select("img[src]");
