...
/**
 * Created by glacier on 14-8-16.
 */
public class LanzhouClass {

    SaveXML savexml = new SaveXML();

    public LanzhouClass( String newspaper ) {
    ...
    public HashMap<String,String> getLayout( String Link ) {
        ...
        try {
            Document Doc = Jsoup.connect(Link)
                    ...
                    .get();
            //获取版面链接
            Elements Layouts = Doc.select("table[width=170]").select("a[href]");
            if ( Layouts.size() == 0 )
                Layouts = Doc.select("table[width=167]").select("a[href]");
    ...
    public void getNewsInfo( String NewsUrl ) { 	//获得新闻来源URL
        try {
            ...
            Document Doc = Jsoup.connect(NewsUrl)
                    ...
                    .timeout(3000)
                    .get();
            Element TitleEle = Doc.select("strong").first();
            ...
            Elements IMGs = Doc.select("table[width=501]").select("img[src]");
