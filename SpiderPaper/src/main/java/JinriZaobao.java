 ...
 */
public class JinriZaobao {

    SaveXML savexml = new SaveXML();

    public JinriZaobao( String newspaper ) {
    ...
    public void getNewsInfo( String NewsUrl ) { 	//获得新闻来源URL
        try {
            ...
            Document Doc = Jsoup.connect(NewsUrl)
                    ...
                    .timeout(3000)
                    .get();
            Element TitleEle = Doc.select("h1[class=main-article-title]").first();
            ...
            Elements IMGs = Doc.select("div[class=main_ar_pic_text]").select("img[src]");
