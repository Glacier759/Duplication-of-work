 ...
 */
public class ZhongguoQiche {

    SaveXML savexml = new SaveXML();

    public ZhongguoQiche( String newspaper ) {
    ...
    public HashMap<String,String> getLayout( String Link ) {
        ...
        try {
            Document Doc = Jsoup.connect(Link)
                    ...
                    .get();
            //获取版面链接
            Elements Layouts = Doc.select("div[class=DN_DownList scY]").select("a[href]");
    ...
    public HashSet<String> getNewsLinks( String LayoutLink ) {
        ...
        try {
            Document Doc = Jsoup.connect(LayoutLink)
                    ...
                    .timeout(3000)
                    .get();
            Elements Areas = Doc.select("div[class=DN_DownList scY]").first().select("a[href]");
