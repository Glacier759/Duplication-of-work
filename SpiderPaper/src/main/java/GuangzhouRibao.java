 ...
 */
public class GuangzhouRibao {

    SaveXML savexml = new SaveXML();

    public GuangzhouRibao( String newspaper ) {
    ...
    public HashMap<String,String> getLayout( String Link ) {
        ...
        try {
            ...
            Elements Dies = Doc.select("div[id=sharelista]").select("a[href]");
            for ( Element Die:Dies ) {
                Doc = Jsoup.connect(Die.attr("abs:href"))
                        .userAgent("Mozilla")
                        .cookie("auth", "token")
                        .timeout(3000)
                        .get();
                Layouts = Doc.select("div[id=page_list]").select("a[href]");
            }
    ...
    public void getNewsInfo( String NewsUrl ) { 	//获得新闻来源URL
        try {
            ...
            Document Doc = Jsoup.connect(NewsUrl)
                    ...
                    .timeout(3000)
                    .get();
            Element TitleEle = Doc.select("h3[id=artTitle]").first();
            String Title = TitleEle.text(); 		//获得文章title

            String PublishTime = getDate(NewsUrl); 	//获得文章发表日期
            Element TextContent = Doc.select("founder-content").first();
            ...
            Elements IMGs = Doc.select("div[id=leftMedia]").select("img[src]");
