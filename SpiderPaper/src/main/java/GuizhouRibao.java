...
/**
 * Created by glacier on 14-8-14.
 */
public class GuizhouRibao {

    SaveXML savexml = new SaveXML();

    public GuizhouRibao( String newspaper ) {
    ...
    public HashMap<String,String> getLayout( String Link ) {
        ...
        try {
            Document Doc = Jsoup.connect(Link.substring(0,Link.lastIndexOf('/')+1)+"PageArticleIndexGB.htm")
                    .userAgent("Mozilla")
                    .cookie("auth", "token")
                    .timeout(3000)
                    .get();
            //获取版面链接
            Elements Layouts = Doc.select("td[class=dotline]").select("a[href]");
    ...
    public HashSet<String> getNewsLinks( String LayoutLink ) {
        ...
        try {
            Document Doc = Jsoup.connect(LayoutLink)
                    ...
                    .timeout(3000)
                    .get();
            Elements Areas = Doc.select("td[width=88%]").select("a[href]");
    ...
    public void getNewsInfo( String NewsUrl ) { 	//获得新闻来源URL
        try {
            ...
            Document Doc = Jsoup.connect(NewsUrl)
                    ...
                    .timeout(3000)
                    .get();
            Element TitleEle = Doc.select("div[class=detailtitle]").first();
            String Title = TitleEle.text(); 		//获得文章title

            String PublishTime = NewsUrl.substring(40,44)+"-"+NewsUrl.substring(44,46)+"-"+NewsUrl.substring(46,48); //获得文章发表日期
            Element TextContent = Doc.select("span[id=contenttext]").first();
            String Content = "\r\n" + TextContent.text().replaceAll(" 　　", "\r\n") + "\r\n"; 	//获得文章正文内容

            List<String> IMGList = new ArrayList<String>(); 		//获得图片地址列表
            Elements IMGs = Doc.select("td[bgcolor=#E8E8D7]").select("img[src]");
