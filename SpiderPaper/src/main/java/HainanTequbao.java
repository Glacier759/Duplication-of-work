 ...
 */
public class HainanTequbao {

    SaveXML savexml = new SaveXML();

    public HainanTequbao( String newspaper ) {
    ...
    public HashMap<String,String> getLayout( String Link ) {
        ...
        try {
            Document Doc = Jsoup.connect(Link)
                    ...
                    .get();
            //获取版面链接
            Elements Layouts = Doc.select("div[id=scrollLayer]").select("a[href]");
    ...
    public void getNewsInfo( String NewsUrl ) { 	//获得新闻来源URL
        try {
            ...
            Document Doc = Jsoup.connect(NewsUrl)
                    ...
                    .timeout(3000)
                    .get();
            Element TitleEle = Doc.select("p[class=title1]").first();
            String Title = TitleEle.text(); 		//获得文章title

            String PublishTime = getDate(NewsUrl); 	//获得文章发表日期
            Element TextContent = Doc.select("td[class=content_tt]").first();
