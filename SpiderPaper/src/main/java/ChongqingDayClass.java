 ...
 */
public class ChongqingDayClass {

    SaveXML savexml = new SaveXML();

    public ChongqingDayClass( String newspaper ) {
    ...
    public HashMap<String,String> getLayout( String Link ) {
        ...
        try {
            Document Doc = Jsoup.connect(Link)
                    ...
                    .get();
            //获取版面链接
            Elements Layouts = Doc.select("tbody").select("td[class=default]");
    ...
    public void getNewsInfo( String NewsUrl ) { 	//获得新闻来源URL
        try {
            ...
            Document Doc = Jsoup.connect(NewsUrl)
                    ...
                    .timeout(3000)
                    .get();
            Element TitleEle = Doc.select("td[width=572]").first();
            ...
            Elements IMGs = Doc.select("table[style=BORDER-BOTTOM: #555 1px solid]").first().nextElementSibling().select("img[src]");
