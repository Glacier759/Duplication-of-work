 ...
 */
public class HubeiRibao {

    SaveXML savexml = new SaveXML();

    public HubeiRibao( String newspaper ) {
    ...
    public HashMap<String,String> getLayout( String Link ) {
        ...
        try {
            Document Doc = Jsoup.connect(Link)
                    ...
                    .get();
            //获取版面链接
            Elements Layouts = Doc.select("table[id=Table1]").select("td[onclick]");
            for (Element Layout:Layouts) {
                String open = Layout.attr("onclick");
                String link = open.substring(open.indexOf('\'')+1, open.indexOf(',')-1);
                LayoutMap.put(Link+link, Layout.text());
            }
    ...
    public void getNewsInfo( String NewsUrl ) { 	//获得新闻来源URL
        try {
            ...
            Document Doc = Jsoup.connect(NewsUrl)
                    ...
                    .timeout(3000)
                    .get();
            Element TitleEle = Doc.select("td[style=font-size:22px; font-weight:bold; color: #0033FF;]").first();
            String Title = TitleEle.text(); 		//获得文章title
            String PublishTime = NewsUrl.substring(34,38)+"-"+NewsUrl.substring(38,40)+"-"+NewsUrl.substring(40,42); //获得文章发表日期
            Elements ContentPTags = Doc.select("div[id=copytext]");
            String Content = "\r\n" + ContentPTags.text().replaceAll("　　", "\r\n") + "\r\n";	//获得文章正文内容
            List<String> IMGList = new ArrayList<String>(); 		//获得图片地址列表
            Elements IMGs = Doc.select("div[id=copytext]").select("img[src]");
