 ...
 */
public class JianchaRibao {

    SaveXML savexml = new SaveXML();

    public JianchaRibao( String newspaper ) {
    ...
    public void getNewsInfo( String NewsUrl ) { 	//获得新闻来源URL
        try {
            ...
            Document Doc = Jsoup.connect(NewsUrl)
                    ...
                    .timeout(3000)
                    .get();
            Element TitleEle = Doc.select("td[class=font01]").first();
            ...
            Elements IMGs = Doc.select("table[style=WIDTH:159PX;]").select("img[src]");
