 ...
 */
public class ShenyangRibao {

    SaveXML savexml = new SaveXML();

    public ShenyangRibao( String newspaper ) {
    ...
    public HashSet<String> getNewsLinks( String LayoutLink ) {
        ...
        try {
            ...
            for ( Element Area:Areas ) {
                ...
                if ( href.contains("?") )
                    NewsLink.add(href.substring(0,href.indexOf('?')));
                else
                    NewsLink.add(href);
            }
        } catch( Exception e ) {
    ...
    public void getNewsInfo( String NewsUrl ) { 	//获得新闻来源URL
        try {
            ...
            Document Doc = Jsoup.connect(NewsUrl)
                    ...
                    .timeout(3000)
                    .get();
            Element TitleEle = Doc.select("strong[id=TitleID]").first();
            ...
            Elements IMGs = Doc.select("table[bgcolor=#ffffff]").select("img[src]");
