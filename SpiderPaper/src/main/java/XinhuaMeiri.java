 ...
 */
public class XinhuaMeiri {

    SaveXML savexml = new SaveXML();

    public XinhuaMeiri( String newspaper ) {
    ...
    public HashMap<String,String> getLayout( String Link ) {
        ...
        try {
            Document Doc = Jsoup.connect(Link)
                    ...
                    .get();
            //获取版面链接
            Elements Layouts = Doc.select("img[height=16]");
            for (Element Layout:Layouts) {
                Element hrefTR = Layout.parent().parent().parent().select("a[href]").first();
                LayoutMap.put(hrefTR.attr("abs:href"), hrefTR.text());
            }
    ...
    public HashSet<String> getNewsLinks( String LayoutLink ) {
        ...
        try {
            Document Doc = Jsoup.connect(LayoutLink)
                    ...
                    .timeout(3000)
                    .get();
            Elements Areas = Doc.select("div[id=mylink]").select("a[href]");
    ...
    public void getNewsInfo( String NewsUrl ) { 	//获得新闻来源URL
        try {
            ...
            Document Doc = Jsoup.connect(NewsUrl)
                    ...
                    .timeout(3000)
                    .get();
            Element TextEle = Doc.select("BODY[bgColor=#ffffff]").first();
            Elements TitleEle = TextEle.select("FONT[style=FONT-SIZE: 23px]");
            String Title = TitleEle.text(); 		//获得文章title

            String PublishTime = NewsUrl.substring(28,32)+"-"+NewsUrl.substring(32,34)+"-"+NewsUrl.substring(34,36); //获得文章发表日期
            Elements ContentPTags = TextEle.select("div");
            String Content = "\r\n"; 						//获得文章正文内容
            for ( Element ContentPTag:ContentPTags ) {
                Content += ContentPTag.text() + "\r\n";
            }
            List<String> IMGList = new ArrayList<String>(); 		//获得图片地址列表
            Elements IMGs = TextEle.select("TD[bgColor=#f7f7f7]").select("img[src]");
