 ...
 */
public class YangziWanbao {

    SaveXML savexml = new SaveXML();

    public YangziWanbao( String newspaper ) {
    ...
    public void getNewsInfo( String NewsUrl ) { 	//获得新闻来源URL
        try {
            ...
            Document Doc = Jsoup.connect(NewsUrl)
                    ...
                    .timeout(3000)
                    .get();
            Element TitleEle = Doc.select("td[class=title1]").first();
            String Title = TitleEle.text(); 		//获得文章title

            String PublishTime = getDate(NewsUrl); 	//获得文章发表日期
            Elements ContentPTags = Doc.select("td[class=content_right]").select("p");
            String Content = "\r\n"; 						//获得文章正文内容
