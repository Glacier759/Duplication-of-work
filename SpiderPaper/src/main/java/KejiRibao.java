 ...
 */
public class KejiRibao {

    SaveXML savexml = new SaveXML();

    public KejiRibao( String newspaper ) {
    ...
    public void getNewsInfo( String NewsUrl ) { 	//获得新闻来源URL
        try {
            ...
            Document Doc = Jsoup.connect(NewsUrl)
                    ...
                    .timeout(3000)
                    .get();
            Element TitleEle = Doc.select("div[class=biaoti]").first();
            ...
            Elements IMGs = Doc.select("div[class=picture]").select("img[src]");
