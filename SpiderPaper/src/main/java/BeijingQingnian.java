 ...
 */
public class BeijingQingnian {

    SaveXML savexml = new SaveXML();

    public BeijingQingnian( String newspaper ) {
    ...
    public HashSet<String> getNewsLinks( String LayoutLink ) {
        ...
        try {
            ...
            for ( Element Area:Areas ) {
                String newsLink = Area.attr("abs:href");
                NewsLink.add(newsLink.substring(0,newsLink.indexOf('?')));
            }
        } catch( Exception e ) {
            e.printStackTrace();
        }
    ...
    public void getNewsInfo( String NewsUrl ) { 	//获得新闻来源URL
        try {
            ...
            Element TextContent = Doc.select("div[class=contnt]").first();
