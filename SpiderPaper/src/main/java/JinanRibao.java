...
/**
 * Created by glacier on 14-8-15.
 */
public class JinanRibao {

    SaveXML savexml = new SaveXML();

    public JinanRibao( String newspaper ) {
    ...
    public HashMap<String,String> getLayout( String Link ) {
        ...
        try {
            Document Doc = Jsoup.connect(Link)
                    ...
                    .get();
            //获取版面链接
            Elements Layouts = Doc.select("a[target=_top]");
    ...
    public HashSet<String> getNewsLinks( String LayoutLink ) {
        ...
        try {
            ...
            for ( Element Area:Areas ) {
                String onclick = Area.attr("onclick");
                NewsLink.add("http://"+new URL(LayoutLink).getHost() + onclick.substring(onclick.indexOf('\'')+1,onclick.indexOf(',')-1));
            }
        } catch( Exception e ) {
            e.printStackTrace();
        }
    ...
    public void getNewsInfo( String NewsUrl ) { 	//获得新闻来源URL
        try {
            ...
            Document Doc = Jsoup.connect(NewsUrl)
                    ...
                    .timeout(3000)
                    .get();
            Element TitleEle = Doc.select("div[class=detail]").select("h2").first();
            String Title = TitleEle.text(); 		//获得文章title

            String PublishTime = NewsUrl.substring(31,35)+"-"+NewsUrl.substring(35,37)+"-"+NewsUrl.substring(37,39); 	//获得文章发表日期
            Elements ContentPTags = Doc.select("div[class=article]");
            String Content = "\r\n" + ContentPTags.text().replaceAll("　　", "\r\n") + "\r\n";		//获得文章正文内容
            List<String> IMGList = new ArrayList<String>(); 		//获得图片地址列表
            Elements IMGs = Doc.select("div[class=article]").select("img[src]");
