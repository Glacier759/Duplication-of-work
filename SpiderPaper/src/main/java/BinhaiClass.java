 ...
 */
public class BinhaiClass {

    SaveXML savexml = new SaveXML();

    public BinhaiClass( String newspaper ) {
    ...
    public void getNewsInfo( String NewsUrl ) { 	//获得新闻来源URL
        try {
            ...
            Element TitleEle = textDIV.select("founder-title").first();
            String Title = TitleEle.text(); 		//获得文章title

            String PublishTime = getDate(NewsUrl); 	//获得文章发表日期
            Element TextContent = textDIV.select("div[id=ozoom]").first();
