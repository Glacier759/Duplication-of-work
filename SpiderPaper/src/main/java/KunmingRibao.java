 ...
 */
public class KunmingRibao {

    SaveXML savexml = new SaveXML();

    public KunmingRibao( String newspaper ) {
    ...
    public void getNewsInfo( String NewsUrl ) { 	//获得新闻来源URL
        try {
            ...
            Document Doc = Jsoup.connect(NewsUrl)
                    ...
                    .timeout(3000)
                    .get();
            Element TitleEle = Doc.select("div[id=content_title]").first();
            ...
            Elements IMGs = Doc.select("div[id=content_content]").select("img[src]");
