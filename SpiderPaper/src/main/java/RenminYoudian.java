 ...
 */
public class RenminYoudian {

    SaveXML savexml = new SaveXML();

    public RenminYoudian( String newspaper ) {
    ...
    public HashMap<String,String> getLayout( String Link ) {
        ...
        try {
            Document Doc = Jsoup.connect(Link)
                    ...
                    .get();
            //获取版面链接
            Elements Layouts = Doc.select("li");
            for (Element Layout:Layouts) {
                Elements href = Layout.select("a[href]");
                Elements img = Layout.select("img[src]");
                if ( href.size() > 0 && img.size() > 0 ) {
                    LayoutMap.put(href.attr("abs:href"), Layout.text());
                }
            }
