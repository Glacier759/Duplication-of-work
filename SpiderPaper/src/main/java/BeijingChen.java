 ...
 */
public class BeijingChen {

    SaveXML savexml = new SaveXML();

    public BeijingChen( String newspaper ) {
    ...
    public void getNewsInfo( String NewsUrl ) { 	//获得新闻来源URL
        try {
            ...
            Document Doc = Jsoup.connect(NewsUrl)
                    ...
                    .timeout(3000)
                    .get();
            Element textDIV = Doc.select("table[height=396]").first();
            Element TitleEle = textDIV.select("td").first();
            String Title = TitleEle.text(); 		//获得文章title

            String PublishTime = getDate(NewsUrl); 	//获得文章发表日期
            Element TextContent = textDIV.select("founder-content").first();
            ...
            Elements IMGs = textDIV.select("img[src]");
