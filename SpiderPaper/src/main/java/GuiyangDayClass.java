 ...
 */
public class GuiyangDayClass {

    SaveXML savexml = new SaveXML();

    public GuiyangDayClass( String newspaper ) {
    ...
    public void getNewsInfo( String NewsUrl ) { 	//获得新闻来源URL
        try {
            ...
            Document Doc = Jsoup.connect(NewsUrl)
                    ...
                    .timeout(3000)
                    .get();
            Element TitleEle = Doc.select("span[class=content-title]").first();
            ...
            Elements IMGs = Doc.select("div[class=epaper_pic]").select("img[src]");
