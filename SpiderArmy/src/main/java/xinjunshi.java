...
public class xinjunshi {
    ...
    public void start() throws Exception {
        ...
        for ( String typeUrl:urlMap.keySet() ) {
            ...
            HashMap<String,String> newsMap = getNewsMap(typeUrl);
            for ( String newsUrl:newsMap.keySet() ) {
                String content = getNewsCont(newsUrl);
                if ( content.length() == 0 )
                    continue;
                count ++;
                File file = new File(new File(new File(new File("armyData"), "新军事"), typeDir), System.currentTimeMillis()+".txt");
